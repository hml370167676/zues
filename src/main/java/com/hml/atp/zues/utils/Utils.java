...
public class Utils {
    ...
    public static String checkHasParas(String url, List<NameValuePair> nvps) {
        // 检测url中是否存在参数
        if (url.contains(QUESTION_MARK) && url.indexOf(QUESTION_MARK) < url.indexOf(EQUAL_SIGN)) {
            Map<String, String> map = buildParas(url.substring(url.indexOf(QUESTION_MARK) + 1));
            map2List(nvps, map);
